...
<body>
  ...
  <table class="ref-table">
    ...
    <tr>
      ...
      <td class="colon">:</td>
      <td>{{ $letter->date->format('j F Y') }}</td>
    </tr>
  </table>
  ...
  @foreach (preg_split('/\n\s*\n/', trim($letter->body_text)) as $paragraph)
    <p>{{ trim($paragraph) }}</p>
  @endforeach

  <div class="signature">
    @if ($letter->signatory && $letter->signatory->signature_path && file_exists(public_path($letter->signatory->signature_path)))
      <div class="signature-image-wrap">
        <img
          class="signature-image"
          src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path($letter->signatory->signature_path))) }}" />
      </div>
    @endif
    <div class="name">{{ $letter->signatory->name ?? '' }}</div>
    <div class="designation">{{ $letter->signatory->designation_en ?? '' }}</div>
    <div class="designation">Kulliyyah of Information and Communication Technology (KICT)</div>
    <div class="designation">International Islamic University Malaysia (IIUM)</div>
  </div>
</body>
</html>
