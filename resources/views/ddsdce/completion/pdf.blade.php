...
<body>
  ...
  <table class="ref-table">
    <tr>
      ...
      <td class="colon">:</td>
      <td>{{ $completionLetter->reference_no }}</td>
    </tr>
    <tr>
  ...
  <table class="detail-table">
    <tr>
      ...
      <td class="colon">:</td>
      <td>{{ strtoupper($completionLetter->student->name) }}</td>
    </tr>
    <tr>
      <td class="detail-label">NRIC / PASSPORT</td>
      <td class="colon">:</td>
      <td>{{ collect([$completionLetter->student->nric_no, $completionLetter->student->passport_no])->filter()->implode(' / ') ?: '-' }}</td>
    </tr>
    <tr>
      <td class="detail-label">MATRIC NO</td>
      <td class="colon">:</td>
      <td>{{ $completionLetter->student->matric_no }}</td>
    </tr>
    <tr>
      <td class="detail-label">PROGRAM</td>
      <td class="colon">:</td>
      <td>{{ strtoupper($completionLetter->student->program->name_en ?? '') }}</td>
    </tr>
  </table>

  @foreach (preg_split('/\n\s*\n/', trim($completionLetter->body_text)) as $paragraph)
    <p>{{ trim($paragraph) }}</p>
  @endforeach

  <div class="signature">
    @if ($completionLetter->signatory->signature_path && file_exists(public_path($completionLetter->signatory->signature_path)))
      <div class="signature-image-wrap">
        <img
          class="signature-image"
          src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path($completionLetter->signatory->signature_path))) }}" />
      </div>
    @endif
    <div class="name">{{ $completionLetter->signatory->name }}</div>
    <div class="designation">{{ $completionLetter->signatory->designation_en }}</div>
    <div class="kulliyyah">{{ $completionLetter->student->department->kulliyyah->name_en ?? '' }}</div>
