@php
    ...
    // Reinstatement Semester is only ever recent — limit the list to the most
    ...
    $reinstateSessionOptions = $academicSessions->take(6);
    if ($record && ! $reinstateSessionOptions->contains('id', $record->academic_session_id)) {
        $extra = $academicSessions->firstWhere('id', $record->academic_session_id);
        if ($extra) {
            $reinstateSessionOptions = $reinstateSessionOptions->push($extra);
        }
    }
@endphp
...
<div class="row">
  ...
  <div class="col-md-4 mb-6">
    <label for="academic_session_id" class="form-label">Reinstatement Semester</label>
    <select id="academic_session_id" name="academic_session_id" class="form-select @error('academic_session_id') is-invalid @enderror" required>
      ...
      @foreach ($reinstateSessionOptions as $session)
        <option value="{{ $session->id }}" {{ (int) old('academic_session_id', $record->academic_session_id ?? '') === $session->id ? 'selected' : '' }}>
          {{ $session->label() }}
        </option>
      @endforeach
    </select>
    <div class="form-text">The semester the student is reinstated in.</div>
  ...
  <div class="col-md-4 mb-6">
    <label for="date" class="form-label">Reinstatement Date</label>
    <input
      type="date"
      id="date"
      name="date"
      class="form-control @error('date') is-invalid @enderror"
      value="{{ old('date', optional($record?->date)->format('Y-m-d')) }}"
      required />
    @error('date')
      <div class="invalid-feedback d-block">{{ $message }}</div>
    @enderror
...
<a href="{{ route('ddsdce.reinstate.index') }}" class="btn btn-outline-secondary">Cancel</a>
